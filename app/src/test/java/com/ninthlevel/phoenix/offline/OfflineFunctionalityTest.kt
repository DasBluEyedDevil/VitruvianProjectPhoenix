package com.ninthlevel.phoenix.offline

import com.ninthlevel.phoenix.data.local.WorkoutDao
import com.ninthlevel.phoenix.data.local.WorkoutSessionEntity
import com.ninthlevel.phoenix.data.local.WorkoutMetricEntity
 ...
 * COMPLETELY OFFLINE without requiring any network servers.
 *
...
class OfflineFunctionalityTest {
    ...
    fun setup() {
        ...
        val personalRecordDao = mockk<com.ninthlevel.phoenix.data.local.PersonalRecordDao>(relaxed = true)
    ...
    fun `test BLE device connection - direct device communication only`() = runTest {
        ...
        every { bleRepository.connectionState } returns MutableStateFlow(
            ConnectionState.Connected("Vee_TEST", deviceAddress)
        )
    ...
    fun `test workout metrics are calculated locally`() = runTest {
        ...
        val metrics = listOf(
            ...
            WorkoutMetric(3, loadA = 18f, loadB = 18f, positionA = 1800f, positionB = 1800f)
        )
