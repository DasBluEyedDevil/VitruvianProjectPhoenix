...
class AMRAPFeatureTest {
    ...
    private lateinit var dataBackupManager: com.ninthlevel.phoenix.util.DataBackupManager
    ...
    fun setup() {
        ...
        every { bleRepository.handleState } returns MutableStateFlow(com.ninthlevel.phoenix.data.ble.HandleState.Released)
    ...
    fun `test 2 - AMRAP set does NOT trigger auto-stop`() {
        ...
        // ACT: Process 16 samples. First call (0,0) is baseline; later samples
        // report working-rep count on repsSetCount, which the machine uses for
        // working reps only. 0..15 with set=i yields 15 confirmed working reps.
        for (i in 0..15) {
    ...
    fun `test 3 - AMRAP set saves actual reps completed when manually stopped`() = runTest {
        ...
        every { bleRepository.connectionState } returns MutableStateFlow(
            ConnectionState.Connected("Vee_TEST", "00:11:22:33:44:55")
        )
    ...
    fun `test 4 - non-AMRAP set still has auto-stop enabled`() {
        ...
        // ACT: Baseline (0,0) then working reps 1..10 on repsSetCount
        for (i in 0..10) {
            actualRepCounter.process(
                repsRomCount = i,
                repsSetCount = i,
