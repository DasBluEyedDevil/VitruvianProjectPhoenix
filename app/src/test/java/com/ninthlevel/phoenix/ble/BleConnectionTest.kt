package com.ninthlevel.phoenix.ble
 ...
 *
 * Tests direct Bluetooth Low Energy communication with trainer devices
...
class BleConnectionTest {
    ...
    fun `test device scanning - local BLE discovery`() = runTest {
        ...
        every { mockDevice.name } returns "Vee_TEST"
        ...
        assertEquals("Vee_TEST", foundDevices[0].device.name)
    ...
    fun `test direct device connection - no server pairing`() = runTest {
        ...
        coEvery { bleRepository.connectToDevice(deviceAddress) } answers {
            stateFlow.value = ConnectionState.Connected("Vee_TEST", deviceAddress)
            Result.success(Unit)
        }
    ...
    fun `test connection state transitions - local management`() = runTest {
        ...
        stateFlow.value = ConnectionState.Connected("Vee_TEST", "AA:BB:CC:DD:EE:FF")
    ...
    fun `test multiple device handling - local preference`() = runTest {
        ...
        every { device1.name } returns "Vee_TEST_1"
        every { device2.name } returns "Vee_TEST_2"
