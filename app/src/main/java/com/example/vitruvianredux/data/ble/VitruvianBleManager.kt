...
class VitruvianBleManager(
    ...
    // Force-based handle detection (matching trainer)
    ...
    // Force-based handle detection parameters (matching trainer exactly)
    ...
    private inner class VitruvianGattCallback : BleManagerGattCallback() {
        ...
        override fun isRequiredServiceSupported(gatt: BluetoothGatt): Boolean {
            ...
            // Get required characteristics (matching trainer)
            ...
            // If characteristics not in NUS service, search all services (matching trainer fallback)
        ...
        override fun initialize() {
            ...
            fun checkAllOperationsComplete() {
                ...
                if (remaining == 0) {
                    ...
                    // The trainer/web app does this - property polling at 500ms intervals
     ...
     * This is how the trainer reads position/force data
    ...
    fun startMonitorPolling() {
        ...
        monitorPollingJob = pollingScope.launch {
            Timber.d("Starting monitor polling (16ms interval / ~60Hz - matching trainer)")
            while (isActive) {
                try {
                    ...
                    delay(16) // Poll every 16ms (~60Hz) - matching trainer
                } catch (e: Exception) {
    ...
    private fun handleMonitorData(data: Data) {
        try {
            ...
            // Parse the monitor data packet (matching device parseMonitorData)
            ...
            // Calculate velocity for handle detection (trainer protocol)
            ...
            // Analyze and update handle state (force-based with duration, matching trainer)
