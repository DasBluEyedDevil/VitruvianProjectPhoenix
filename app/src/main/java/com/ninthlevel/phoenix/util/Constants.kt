...
object WorkoutConstants {
    ...
    // Position ranges (mm) - from trainer documentation
...
/**
 * Protocol constants - aligned with device protocol (trainer)
 * NOTE: Legacy protocol used different sizes and commands
 */
@Suppress("unused")  // Protocol reference constants
object ProtocolConstants {
    // Command types (single-byte commands per device protocol)
    ...
    // Legacy command IDs (legacy command IDs)
    ...
    // Frame sizes (device protocol aligned)
    const val STOP_PACKET_SIZE = 2
    const val REGULAR_PACKET_SIZE = 25        // legacy size 96
    const val ECHO_PACKET_SIZE = 29           // legacy size 40
