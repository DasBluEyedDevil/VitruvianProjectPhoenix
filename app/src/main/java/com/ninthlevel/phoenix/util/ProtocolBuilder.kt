...
/**
 * Builds binary protocol frames for trainer communication.
 */
object ProtocolBuilder {
     ...
     * CRITICAL: Program-mode command is 0x04
     */
    fun buildProgramParams(params: WorkoutParameters): ByteArray {
        ...
        // Header section - Command 0x04 for PROGRAM mode (from observed device traffic)
        frame[0] = 0x04
        frame[1] = 0x00
        frame[2] = 0x00
        frame[3] = 0x00
        ...
        // Some constant values from the observed device traffic
        frame[5] = 0x03
        frame[6] = 0x03
        frame[7] = 0x00
        ...
        // Fill in some other fields from the observed device traffic
     ...
     * device protocol uses 0x50, but our firmware responds to 0x05
     */
    ...
    /**
     * Build the 0x50 stop packet (2 bytes)
     * Per trainer analysis (ANALYSIS_JUSTLIFT.md):
     * - The trainer uses StopPacket (0x50 0x00) to end sessions and CLEAR FAULTS
     ...
     */
    fun buildStopPacket0x50(): ByteArray {
        return byteArrayOf(0x50, 0x00)
    }

    /**
     * Build the RESET command (4 bytes)
     * Legacy stop uses 0x0A (same as init).
