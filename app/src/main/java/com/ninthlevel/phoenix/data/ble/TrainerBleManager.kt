...
package com.ninthlevel.phoenix.data.ble

import android.bluetooth.BluetoothGatt
import android.bluetooth.BluetoothGattCharacteristic
import android.content.Context
import com.ninthlevel.phoenix.domain.model.DiagnosticDetails
import com.ninthlevel.phoenix.domain.model.HeuristicPhaseStatistics
import com.ninthlevel.phoenix.domain.model.HeuristicStatistics
import com.ninthlevel.phoenix.domain.model.SampleStatus
import com.ninthlevel.phoenix.domain.model.WorkoutMetric
import com.ninthlevel.phoenix.util.BleConstants
import com.ninthlevel.phoenix.util.WorkoutConstants
...
/**
 * BLE Manager - Handles BLE communication with device
 * Uses Nordic BLE Library for robust BLE operations
 */
@OptIn(ExperimentalStdlibApi::class)
class TrainerBleManager(
    context: Context,
    private val connectionLogger: com.ninthlevel.phoenix.data.logger.ConnectionLogger? = null
    ...
    // Workout command characteristics (for testing)
    ...
    /**
     * Enable or disable strict validation mode (matching trainer timing).
    ...
    override fun log(priority: Int, message: String) {
        Timber.tag("TrainerBLE").log(priority, message)
    }
    ...
    override fun getGattCallback(): BleManagerGattCallback {
        return TrainerGattCallback()
    }

    /**
     * Custom GATT callback for device
     */
    private inner class TrainerGattCallback : BleManagerGattCallback() {
        ...
        override fun isRequiredServiceSupported(gatt: BluetoothGatt): Boolean {
            ...
            // Collect ALL characteristics for notifications (matching observed trainer timing)
            ...
            // Collect workout command characteristics for testing alternate command characteristics
        ...
        private fun tryReadDeviceVersion(gatt: BluetoothGatt) {
            try {
                ...
                if (versionChar == null) {
                    Timber.d("VERSION characteristic not found - cannot read version info")
                    return
                }
                    ...
                    .with { _, data ->
                        try {
                            ...
                            if (bytes != null && bytes.isNotEmpty()) {
                                ...
                                Timber.i("╔════════════════════════════════════════════════════════════╗")
                                Timber.i("║  TRAINER VERSION CHARACTERISTIC (READ)                   ║")
                                ...
                                connectionLogger?.log(
                                    ...
                                    deviceName = currentDeviceName,
                                    deviceAddress = currentDeviceAddress,
                                    message = "VERSION characteristic read",
        ...
        override fun initialize() {
            ...
            fun checkAllOperationsComplete() {
                ...
                if (remaining == 0) {
                    ...
                    // The trainer does this - property polling at 500ms intervals (matches trainer timing)
            ...
            // Enable notifications on ALL required characteristics (matching observed trainer timing)
            ...
            for (characteristic in notifyCharacteristics) {
                ...
                // Per device protocol, Sample data MUST be polled via readCharacteristic().
     ...
     *
     * NEW APPROACH (matches trainer timing):
    ...
    fun startMonitorPolling(forAutoStart: Boolean = false) {
        ...
        monitorPollingJob = pollingScope.launch {
            Timber.i("🔄 Starting SEQUENTIAL monitor polling (suspend-based, matches trainer timing)")
            ...
            while (isActive) {
                try {
                    ...
                    // This matches observed trainer behavior - no fixed timer, callback-driven
     ...
     * Matches trainer timing interval - Uses suspend-based reads 
     */
    fun startDiagnosticPolling() {
        propertyPollingJob?.cancel()
        propertyPollingJob = pollingScope.launch {
            Timber.d("🔄 Starting SEQUENTIAL diagnostic polling (500ms interval - matches trainer timing)")
            ...
            while (isActive) {
                try {
                    ...
                    // CRITICAL: Suspend until read completes (matches trainer timing)
                    ...
                    delay(500) // Poll every 500ms (Trainer interval - verified)
    ...
    /**
     * Start polling heuristic characteristic every 250ms (4Hz) - matching trainer timing.
    ...
    fun startHeuristicPolling() {
        ...
        heuristicPollingJob = pollingScope.launch {
            Timber.d("Starting SEQUENTIAL heuristic polling (250ms interval / 4Hz - matching trainer timing)")
            while (isActive) {
                try {
                    ...
                    delay(250) // Poll every 250ms (4Hz) - matching trainer timing
                } catch (e: Exception) {
    ...
    /**
     * Stop all polling (matching trainer timing)
     */
    fun stopPolling() {
        val timestamp = System.currentTimeMillis()
     ...
     * (Matching trainer timing force-based detection)
     */
        ...
        return try {
            ...
            nusRxCharacteristic?.let { characteristic ->
                ...
                writeCharacteristic(characteristic, data, BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE)
                    // REMOVED .split() - protocol requires exact frame sizes!
     ...
     */
    @Suppress("DEPRECATION")
    suspend fun testAlternateCommandCharacteristics(): Result<Unit> = kotlinx.coroutines.withContext(Dispatchers.Main) {
        try {
            ...
            val programFrame = com.ninthlevel.phoenix.util.ProtocolBuilder.buildProgramParams(
                com.ninthlevel.phoenix.domain.model.WorkoutParameters(
                    workoutType = com.ninthlevel.phoenix.domain.model.WorkoutType.Program(
                        com.ninthlevel.phoenix.domain.model.ProgramMode.OldSchool
                    ),
    ...
    private fun handleMonitorData(data: Data) {
        try {
            ...
            // Validate sample (matching trainer timing)
