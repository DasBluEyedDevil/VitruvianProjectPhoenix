...
import com.ninthlevel.phoenix.data.ble.BleConnectionState
import com.ninthlevel.phoenix.data.ble.DiscoveredDevice
import com.ninthlevel.phoenix.data.ble.HandleState
import com.ninthlevel.phoenix.data.ble.KableBleManager
import com.ninthlevel.phoenix.data.ble.KableBleScanner
import com.ninthlevel.phoenix.data.ble.RepNotification
...
class KableBleRepositoryImpl @Inject constructor(
    ...
    override suspend fun sendInitSequence(): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            Timber.d("sendInitSequence - deprecated (trainer doesn't use 0x0A handshake)")
    ...
    override suspend fun stopWorkout(): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            // Send RESET command (0x0A) - matches observed trainer timing
    ...
    override suspend fun sendStopCommand(): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            // Send StopPacket (0x50) - trainer stop command
    ...
    override suspend fun testAlternateCommandCharacteristics(): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            Timber.d("Testing alternate command characteristics (not implemented in Kable version)")
